...
linkedInURLsDay4 = ['www.google.com']
...
cardList = trello.Lists(ApiKey, token).get_card(day4["id"])
for card in cardList:
    nameListDay4.append(card["name"])

# append the posts extension to URL
for name in nameListDay4:
    URLStart = re.search("https://+", name) # find linkedIn URL by searching for the start point of https://
    if URLStart:
        linkedInURLsDay4.append(name[URLStart.start():len(name)])

# Opens new tab for all linkedIn URLs
for URL in linkedInURLsDay4:
